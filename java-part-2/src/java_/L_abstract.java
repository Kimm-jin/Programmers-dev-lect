...
// 즉, 추상클래스 자체로 객체를 생성할수 없으며, 반드시 이 클래스를 상속받은 자식클래스를 통해서만 객쳋를 생성할 수 있다.
// 추상 클래스는 추상 메서드뿐만 아니라 일반 메서드도 포함할 수 있으며, 필드(맴버 변수)를 가질 수도 있다.
...
// 추상 메서드 : 선언만 있고, 구현은 없으며, 반드시 자식 클래스에서 오버라이딩 되어야 한다.

// * 단점
// 1. 상속의 제약
// - 단일 상속의 제한
// 자바는 단일 상속만 지원하므로, 한 클래스가 두 개 이상의 추상 클래스를 상속 받을 수 없다.
// 이 경우 인터페이스를 사용해야 하지만, 인터페이스는 구현을 가질 수 없다.
// 2. 구현의 강제성
// - 모든 자식 클래스에서 구현 강제
...
abstract class L_animal{
    ...
    // 추상 메서드 : 하위 클래스가 이 메서드를 구현해야 함(오버라이딩, 재정의) -> 강제
