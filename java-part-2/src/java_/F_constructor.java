...
// 4. 기본 생성자 : 클래스에 생성자를 명시하지 않으며, 컴파일러가 매개변수가 없는 기본 생성자를 자동으로 추적한다.
// 하지만 사용자가 하나라도 생성자를 정의하면, 컴파일러는 기본 생성자를 자동으로 추가하지 않는다.
// person1을 보면 정의된 기본생성자를 지워도 에러가 안나며 컴파일러가 기본 생성자를 추가해주었기 때문이다.
// 5. 객체 초기화 : 생성자는 객체의 필드를 초기화하는데 사용된다.
...
//        매개변수가 있는 생성자를 사용하여 객체 생성
        F_person2 person2 = new F_person2("Paul2", 202);
        person2.display();
        ...
        // 이름만 초기화하는 생성자
        F_person3 person3_1 = new F_person3("Paul");
        person3_1.display();

        // 이름과 나이를 초기화하는 생성자
        F_person3 person3_2 = new F_person3("Paul",20);
        person3_2.display();
        ...
        // 첫 줄 생성자 호출 가능은 java 구버전 문법, java22 프리뷰 -> java 25부터 첫줄에서 호출 안해도 된다.
