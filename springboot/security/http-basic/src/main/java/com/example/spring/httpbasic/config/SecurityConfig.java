...
// DI, 라이프사이클 관리 등 스프링 기능을 다 써야 하기 떄문이다.

// 필터는 톰캣에 등록돼야 하는데, 정작 실행하고 싶은 로직은 스프링 Bean이다. - 톰캣에 직접 스프링 Bean을 필터로 꽃을 수는 없다.
// 그래서 DelegatingFilterProxy 클래스간 중간에서 다리를 놓는다.
...
// 4. 성공하면 SecuritCOntext에 인증 정보를 저장하고 다음 필터로 넘어간다.
...
// 그래서 사용자 구역에 로그인한 자격증명을 관리자 구역에 자동으로 재사용하지
...
public class SecurityConfig {
    ...
    // 이 Bean의 역할은 "비밀번호를 어떻게 비교/암호화할지"를 정의한다.
