...
public class F_AccountBookImpl implements F_AccountBook {

    private Map<String, List<F_item>> data = new HashMap<>();
    private Scanner sc;

    public F_AccountBookImpl(Scanner sc){
        this.sc=sc;
    }
    ...
    @Override
    public void showAccount() { // 2. 내역 조회
        if(data.isEmpty()){
            System.out.println("기록이 없습니다.");
            return;
        }
        System.out.println("=== 기록된 날짜 ===");
        List<String> dates = new ArrayList<>(data.keySet()); // key만 뽑기 위함(date)
        ...
        String date = sc.nextLine().trim(); // trim() - 앞뒤 공백 제거
    ...
    public void deleteItem() {
        ...
        List<F_item> items = data.get(date); // 해당 날짜의 기록 모두를 삭제하기 위함
