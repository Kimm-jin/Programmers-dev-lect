...
// Map 인터페이스를 구현하는 대표적 클래스들은 HashMap, TreeMap등이 있다.
...
public class A_collections_map {
    ...
    public static void exam1() {
        ...
        hashMap.put("apple", 40); // 중복 시 덮어씌움
        ...
        // 키의 존재 여부 - return boolean
        boolean containsed = hashMap.containsKey("orange");
        System.out.println("containsed : " + containsed);

        // 순회 방법 1 : entrySet() 방식 - key와 value를 한쌍으로 가져옴
        for (Map.Entry<String, Integer> entry : hashMap.entrySet()) {
            System.out.println(entry.getKey() + " : " + entry.getValue());
        }

        // 순회 방법 2 : keySet() 방식 - key만 모아놓은 set
        // 값을 보고싶으면 get(key)
        for (String key : hashMap.keySet()) {
            System.out.println(key + " : " + hashMap.get(key));
        }

        // 순회 방법 3 : values() 방법 - 값을 확인만 가능
