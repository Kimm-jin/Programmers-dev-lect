...
public class A_collections_queue {
    ...
    public static void exam2_2() {
        // 나이를 기준으로 오름차순 정렬
//        Queue<A_person> queue = new PriorityQueue<>(Comparator.comparing(A_person::getAge));
        // 위와 같은 기능 , 편한걸로 골라쓰면 됨
        Comparator<A_person> ageComparator = new Comparator<A_person>() {
            @Override // 인터페이스라 생성과 동시에 재정의하면 가능
...
// 숫자나 문자는 오름차순 내림차순 등의 정리법이 있지만 아래와 같은
// 객체는 별도의 Compare를 정의해줘야 한다.
class A_person {
    String name;
    int age;
