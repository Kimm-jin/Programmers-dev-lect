...
// - "범용 AOP를 직접 만들 것이나 vs 웹 예외 처리에 특화된 전용 도구를 쓸 것이냐"의 문제다
...
// - AOP로 직접 하면 예외를 잡은 뒤 상태코드 세팅, JSON 직렬화, COntent-type 협상을 전부 손으로 짜야 한다.
// - @RestControllerAdvice는 ResponseEntity / 메시지 컨버터(DTO 자동 JSON 변환) / 예외 타입 매핑을 이미 제공한다.

// 3) 예외 타입별 분기가 선언적이다.
// - @ExceptionHandler(타입) 로 "이 예외는 이 메서드가를 어노테이션으로 나눈다.
...
public class GlobalExceptionHandler {
    ...
    public ErrorResponseDto handleBoardAccessDenied(BoardAccessDeniedException e) {
