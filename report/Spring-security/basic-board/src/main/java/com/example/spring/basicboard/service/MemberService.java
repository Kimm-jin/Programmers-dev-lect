...
// log.debug("가입 요청 : " + dto.getUserId()); // (X) - 로그 레벨이 꺼져 있어도 문자열 연결 비용은 발생
...
// - 서비스 로그 : "비지니스에 무슨일이 있었나" (선별적/의미 - 분기점, 상태 변화, 실패 지점만)
...
public class MemberService {
    ...
    public void join(MemberJoinRequestDto dto) {
        ...
        memberRepository.save( memberMapper.toEntity(dto, encodedPassword) );
    ...
    public Optional<Member> login(LoginRequestDto dto) {
        ...
        if (result.isEmpty()) {
            ...
            log.info("로그인 성공 : userId={}", dto.getUsername());
        }
